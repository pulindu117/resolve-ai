...
def delete_document(source: str, persist_dir: str = "./data/chroma") -> None:
    """
    Delete all chunks belonging to a specific source document.
    Used when a document is removed from the knowledge base.
    """
    client = get_client(persist_dir)
    collection = get_collection(client)
    ...
    if results["ids"]:
        ...
        print(f"Deleted {len(results['ids'])} chunks for source: {source}")
    else:
